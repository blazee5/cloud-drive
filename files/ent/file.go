...
func (*File) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case file.FieldID, file.FieldDownloadCount:
			values[i] = new(sql.NullInt64)
		case file.FieldName, file.FieldUserID, file.FieldContentType:
	...
	for i := range columns {
		...
		case file.FieldDownloadCount:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				f.DownloadCount = int(value.Int64)
			}
		default:
...
func (f *File) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", f.DownloadCount))
	builder.WriteByte(')')
